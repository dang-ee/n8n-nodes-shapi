...
function buildGvimCommand(
	...
	const quoted = quoteUnix(filePath);
...
function buildHelperHtml(params: { command: string; timeout: number; shapiUrl: string }): string {
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		for (let i = 0; i < items.length; i++) {
			try {
				...
				// Build gVim command using global function
				const gvimCommand = buildGvimCommand(filePath, {
					line,
					column,
					encoding,
					createIfMissing,
				...
				// Build helper HTML using global function
				const helperHtml = buildHelperHtml({
					command: gvimCommand,
